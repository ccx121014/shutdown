<#
.SYNOPSIS
Close process that occupies specified port (Auto mode)

.DESCRIPTION
This script automatically closes processes that occupy specified port without user confirmation

.PARAMETER Port
Specify the port number to release

.EXAMPLE
Close-PortProcess-Auto -Port 8080
Close process that occupies port 8080 automatically
...
# Main script logic
Write-Host "========================================"
Write-Host "Port Occupation Process Close Tool (Auto)"
Write-Host "========================================"
...
# 3. Auto close processes (no confirmation)
Write-Host "Auto closing processes..."
$allSuccess = $true
foreach ($processId in $pids) {
